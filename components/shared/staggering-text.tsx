...
import { useInViewLoop } from "@/lib/utils/use-in-view-loop";
          ...
          {chunks.map((letter, index) => {
            ...
            return (
              <motion.span
                ...
                onAnimationStart={() => {
                  if (index === 0) onAnimationStart?.();
                }}
                onAnimationComplete={() => {
                  if (index === chunks.length - 1) onAnimationComplete?.();
                }}
 ...
 *
 * Turns the transition into an animation that flips to the
 * other side when the element receives a mouseenter or mouseleave event.
 */
export function HoverButton({ children = "Devouring Details" }: { children?: string }) {
